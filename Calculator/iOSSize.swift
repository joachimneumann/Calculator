...
struct iOSSize: View {
    ...
    var body: some View {
        ...
        let appFrame = CGSize(
            width: size.width * horizontalFactor,
            height: size.height * verticalFactor)
        let _ = (numberOfCharactersModel.reset(newHeight: Float(size.height)))
        if numberOfCharactersModel.calibrated {
            let _ = print("iOSSize \(size) \(numberOfCharactersModel.numberOfCharacters ?? -1)")
        }
            ...
            ZStack {
                if (numberOfCharactersModel.calibrated) {
                    MainView(brain: brain, t: t)
                        .padding(.leading,   leadingPaddingNeeded ? size.width * fraction : 0)
                        .padding(.trailing, trailingPaddingNeeded ? size.width * fraction : 0)
